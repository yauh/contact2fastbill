...
Version: 0.1 - 2014-07-01
Author URI: http://yauh.de/
*/

function wpcf7_contact2fastbill(&$WPCF7_ContactForm)
{
    // Check whether this form should be processed at all
    if ($WPCF7_ContactForm->posted_data['_wpcf7'] !== get_option('contact2fastbill_formId'))
        return;
    ...
    // these depend on your contact form
    $customerEmail = $WPCF7_ContactForm->posted_data[get_option('contact2fastbill_customerEmail')];
    $salutation = $WPCF7_ContactForm->posted_data[get_option('contact2fastbill_salutation')];
    if ($salutation == 'Herr') {
        $salutation = 'mr';
    } else if ($salutation == 'Frau') {
        $salutation = 'mrs';
    }
    $firstName = $WPCF7_ContactForm->posted_data[get_option('contact2fastbill_firstName')];
    $lastName = $WPCF7_ContactForm->posted_data[get_option('contact2fastbill_lastName')];
    $phone = $WPCF7_ContactForm->posted_data[get_option('contact2fastbill_phone')];
    ...
    $address = $WPCF7_ContactForm->posted_data[get_option('contact2fastbill_address')];
    $zipCode = $WPCF7_ContactForm->posted_data[get_option('contact2fastbill_zipCode')];
    $city = $WPCF7_ContactForm->posted_data[get_option('contact2fastbill_city')];
    ...
    if (count($result['RESPONSE']['CUSTOMERS']) == 0) { // then either create new customer
        ...
        // TODO: Additional info is hard-wired to Message form field - should be configurable or somehow more flexible
        $WPCF7_ContactForm->posted_data['Message'] .= "\n\nKunde in FastBill angelegt (ID: $customerId)";
    } else { // or skip customer creation
        $customerId = $result['RESPONSE']['CUSTOMERS'][0]['CUSTOMER_ID'];
        $customerNumber = $result['RESPONSE']['CUSTOMERS'][0]['CUSTOMER_NUMBER'];
        // TODO: Additional info is hard-wired to Message form field - should be configurable or somehow more flexible
        $WPCF7_ContactForm->posted_data['Message'] .= "\n\nAnfrage eines bestehenden Kunden aus FastBill (ID: $customerId, Kdnr.: $customerNumber)";
    }
